  ...
  perSystem = {
    ...
    packages.neovim = let
      src = pkgs.fetchFromGitHub {
        owner = "neovim";
        repo = "neovim";
        inherit (inputs.neovim-src) rev;
        hash = inputs.neovim-src.narHash;
      };
    ...
    in
      ...
      .overrideAttrs (oa: {
        ...
        preConfigure = ''
          ${oa.preConfigure}
          sed -i cmake.config/versiondef.h.in -e 's/@NVIM_VERSION_PRERELEASE@/-dev+${inputs.neovim-src.shortRev or "dirty"}/'
        '';
      });
  };
}
